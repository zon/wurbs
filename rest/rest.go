...
func New(deps Deps, authMiddleware func(http.Handler) http.Handler) *gin.Engine {
	...
	authHandler := handler.NewAuth(handler.Deps{DB: deps.DB, NATS: deps.NATS})
	...
	channelHandler := handler.NewChannel(handler.Deps{DB: deps.DB, NATS: deps.NATS})
	...
	memberHandler := handler.NewMember(handler.Deps{DB: deps.DB, NATS: deps.NATS})
	...
	messageHandler := handler.NewMessage(handler.Deps{DB: deps.DB, NATS: deps.NATS})
	...
	userHandler := handler.NewUser(handler.Deps{DB: deps.DB, NATS: deps.NATS})
